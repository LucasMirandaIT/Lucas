...
export class DdsValidatorsComponent {
    ...
    static pis(): ValidatorFn {
        return (control: AbstractControl): {[key: string]: any} => {
            ...
            const digitoVerificador = Number(pis.substring(10, 11));
        ...
        return (control: AbstractControl): {[key: string]: any} => {
            ...
            if (dateFormat.test(date)) {
                ...
                if (dateArray[2].length < 4) {
                    // correct year value
                    dateArray[2] = (
                        parseInt(dateArray[2], 10) < 50) ? 2000 +
                        parseInt(dateArray[2], 10) : 1900 +
                        parseInt(dateArray[2], 10
                    );
                }

                const testDate = new Date(dateArray[2], dateArray[1], dateArray[0], 12, 0, 0, 0);
                if (
    ...
    static dateTime(): ValidatorFn {
        return (control: AbstractControl): {[key: string]: any} => {
            ...
            const regex = /^\d{1,2}[\.|\/|-]\d{1,2}[\.|\/|-]\d{1,4} \d{1,2}:\d{1,2}:\d{1,2}$/;
            if (regex.test(dateTime)) {
                const dateArray = dateTime.split(' ');
                const newControlDate: any = {value: dateArray[0]};
                const newControlTime: any = {value: dateArray[1]};

                error = DdsValidatorsComponent.date()(newControlDate) !== null || DdsValidatorsComponent.time()(newControlTime) !== null;
            } else {
                error = true;
            }
